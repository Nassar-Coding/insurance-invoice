...
The original submission was built inside the stated six-to-eight-hour cap across a week; everything after it was done in the revisit window the organisers granted, as gated iterations each with a numeric exit criterion. AI assistance was used throughout and is disclosed in `docs/ai_usage.md`: Codex for acquisition and the original implementation, Claude Code for the gated rebuild, Claude conversationally for diagnosis and for the plan the gates implement. Every prompt, one per gate included, is in `prompts/` verbatim, with what changed between iterations in `prompts/README.md`.
