# Insurance auditing - implementation report

**LLM contract-to-schema with deterministic pricing.** All five supplied contracts are represented by finite, source-referenced records and executed by one fixed Python interpreter. Complete opinions are emitted only where the necessary evidence supports the corrected amount.

Independent verification confirmed both corrected defects closed, 75 passing tests, clean replay and no material regressions. The [original audit reports](../governance/audits/README.md) and before/after evidence are retained unchanged. The source snapshot is `6fee1da60b74512156637a22be15d996a36627e1`. Contract schemas, mappings and confidence policy were unchanged by the two fixes.

The findings and comparisons below describe those technical corrections. [final_reproduction_result.md](final_reproduction_result.md) records the latest repository verification; [evaluation_report.md](evaluation_report.md) contains per-category results and failure analysis.

## Findings, decisions and before/after evidence

| Finding | Audit concern and reproduced original behavior | Decision and implemented correction | Corrected behavior and evidence |
|---|---|---|---|
...
- The current decision log remains one page and the existing write-up two pages. The documents are rendered from Markdown with separately pinned optional dependencies and checked for page count, text and layout.
...
The historical corrected runs are H1 `a1ee1b9f209f4f41913349e165fe8a26` and targets `4d449a6fc6fc49399da2cd567bdb1b51`. They are retained in the history archive. Fresh execution creates new attempts; the latest checks are documented in [final_reproduction_result.md](final_reproduction_result.md).
