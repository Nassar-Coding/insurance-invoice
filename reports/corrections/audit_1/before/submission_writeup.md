...
Tests cover source/schema rejection, dates and amendments, staged rounding, strict thresholds, bundles/exclusions, limits, context revision, unchanged-total faults, whole-row abstention, metrics and failed/stale export handling. Full-snapshot permutation testing found a trace-order defect in conflicting headers; opinion fields were unchanged. The trace list was stabilized and the failure retained. Current implementation evidence also checks label isolation, source bindings and clean replay. These are author implementation checks, not the later independent challenge.
