...
`reports/decision_log.pdf` and `reports/submission_writeup.pdf` are rendered from their current markdown by `tools/render_documents.py`, which asserts each fits its page limit; `reports/document_render.json` records both files' hashes so a reader can confirm they match. The markdown is authoritative. Rendering is optional and uses `requirements-docs.txt`; those packages are not runtime dependencies.
