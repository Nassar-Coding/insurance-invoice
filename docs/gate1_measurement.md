...
Later gates append entries rather than overwrite the baseline; the registration script refuses to overwrite later entries. Tags are user-managed. This page describes Gate 1, where all prediction modules, source data, mappings, schemas, confidence policy and the submission were unchanged and its measurement tests ran with `python tools/run_checks.py gate1_rebuild`. Clean-checkout verification was completed at the Final Gate; the current commands and results are in the [README](../README.md) and [EVALUATION_REPORT.md](../EVALUATION_REPORT.md).
